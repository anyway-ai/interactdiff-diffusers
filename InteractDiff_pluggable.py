...
from InteractDiff_pluggable import SDXL_Unet #change to SDXL model
import itertools
from natsort import natsorted
import numpy as np
from modules import shared
from mics_modules import GatedSelfAttentionDense, FourierEmbedder
from diffusers.models.transformer import Transformer2DModel
...
class PluggableInteractDiffusion:
    def __init__(self, ori_unet:SDXL_Unet):
        super().__init__()
        self.proxy_blocks=[]
        ...
        for block_idx, unet_block in enumerate(itertools.chain(ori_unet.input_blocks, [ori_unet.middle_block], ori_unet.output_blocks)):
            ...
            for module in unet_block.modules():
                if type(module) is Transformer2DModel:
                    Transformer2DModel = module
                    for basic_transformer_block in Transformer2DModel.transformer_blocks:
                        cur_proxy_block = ProxyBasicTransformerBlock(self, basic_transformer_block)
                        cur_proxy_block.initialize_fuser()
...
#example of how this code can be driven

if __name__ == "__main__":

    SDXL_Unet = SDXL_Unet()
    interact_diff = PluggableInteractDiffusion(SDXL_Unet)


    # SAMPLE IMPLEMENTATION
    # #objs = (subject_boxes_tensor.unsqueeze(0), 
    ...
    objs = ()
    interact_diff.update_objs(objs)
    interact_diff.attach_all()
    args = ()
    #args = ((x, timesteps=timesteps, context=context, y=y, **kwargs)
    interact_diff.forward(args)